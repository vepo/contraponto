...
description: Run local static analysis (no SonarCloud token) and fix findings with conservative, behavior-preserving changes.
---
...
GITHUB_ACTIONS=true mvn -B compile -Dmaven.compiler.showWarnings=true -Dmaven.compiler.showDeprecation=true
```
...
| Spotless / formatter drift | `mvn spotless:apply` then re-check | Hand-editing formatting outside the formatter |
...
```bash
GITHUB_ACTIONS=true mvn -B verify
```

If Spotless fails: `mvn spotless:apply` then re-run checks.

If a test fails:

- Fix the **root cause** (production bug or outdated test) — same discipline as [fix_tests.md](fix_tests.md).
- Do **not** proceed to the next issue until `verify` is green.

Re-run `GITHUB_ACTIONS=true mvn -B verify` after large batches or before declaring done.
...
- `GITHUB_ACTIONS=true mvn -B spotless:check`
- `GITHUB_ACTIONS=true mvn -B compile` with no warnings you can fix
...
Note: CI still uploads to SonarCloud separately ([`.github/workflows/build.yml`](../../.github/workflows/build.yml)). This command does not require or wait on that upload.
