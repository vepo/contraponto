...
description: Automatically raise test coverage to 80% (instruction and branch) while ensuring every test asserts all results.
---
...
1. **Measure current coverage**  
   Run `GITHUB_ACTIONS=true mvn clean test` then `mvn jacoco:report` (JaCoCo is configured in `pom.xml`; report at `target/jacoco-report/jacoco.xml` and HTML under `target/jacoco-report/`).  
...
4. **After covering one class** (its missed branches and lines addressed), run the full coverage measurement again (`GITHUB_ACTIONS=true mvn clean test` then `mvn jacoco:report`).
