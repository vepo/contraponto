...
| **Blog subdomain** | Author-scoped host `{username}.{base-domain}` where the main blog home is `/` and posts are `/post/{slug}`; secondary blogs use `/{blogSlug}/…`. | `BlogSubdomainFilter`, `BlogPublicUrlService`, `APP_BLOG_SUBDOMAIN_BASE_DOMAIN` |
