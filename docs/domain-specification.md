...
| **Fediverse follow** | Remote user on another ActivityPub server sends a **Follow** activity to the author's **inbox**; Contraponto **auto-accepts** verified follows and delivers historical main-blog posts. Authors may still **Reject** a pending request from the appearance panel if one exists (e.g. seeded tests). Pending rows show **remote display name**, **Fediverse remote handle**, and actor URL after **remote actor profile fetch**. | `ActivityPubInboxService` |
