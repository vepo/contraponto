...
| **Blog subdomain** | Author-scoped host `{username}.{base-domain}` where the main blog home is `/` and posts are `/post/{slug}`; secondary blogs use `/{blogSlug}/…`. Author workspace hubs (`/writing`, `/manage`, `/administration`, …) are served on the same host without redirecting to the platform. | `BlogSubdomainFilter`, `BlogPublicUrlService`, `APP_BLOG_SUBDOMAIN_BASE_DOMAIN` |
