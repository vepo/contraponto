...
| **ActivityPub federation** | Server-to-server syndication: Contraponto exposes **ActivityPub actors** and delivers **Create** / **Update** / **Delete** activities when authors publish on **any of their blogs** (main and secondary) while **Fediverse opt-in** is on. Distinct from in-app **blog audience Follow** and from the optional **Mastodon profile URL** on author appearance. | `activitypub` package; ADRs [0006](adr/0006-activitypub-federation.md)–[0008](adr/0008-activitypub-actor-identity.md) — ADR-0008 **Reopened** (2026-07-08) for multi-blog outbox/syndication amend; awaiting re-accept |
| **Fediverse actor** | ActivityStreams **Person** — **one per `User`** (not per blog); has `inbox`, `outbox`, `followers`, and `publicKey`. Served as JSON-LD at the actor URL on the user's **blog subdomain**. **Multi-blog syndication:** the same Person's outbox and deliveries carry published posts from **all** of the author's blogs; every Create's object uses the **same** `attributedTo` (that Person). Future: same actor may carry highlight/comment activities (deferred). When the author has a **profile picture**, the actor exposes it as ActivityStreams **`icon`** (`type: Image`) with an absolute image URL. | `ActivityPubActor` |
...
| **Fediverse opt-in** | Author enables federation on **Author appearance** with **one control** covering **all blogs** of that author (main and secondary); when off, actor endpoints return **404** and Creates are not delivered. | Author appearance — Fediverse section |
| **Fediverse follow** | Remote user on another ActivityPub server sends a **Follow** activity to the author's **inbox**; Contraponto **auto-accepts** verified follows and delivers the author's **federated archive** (**Fediverse follow backfill**). **Undo** of Follow marks the edge **REJECTED** (unfollow). A later **Follow** from the same remote **reopens** that edge and auto-accepts again (Accept + backfill). **WebFinger** resolves `acct:` handles and HTTPS profile URLs (`/authors/{username}`, author-subdomain root). **`following`** collection is exposed (empty). | `ActivityPubInboxService`, `ActivityPubWebFingerService`, `ActivityPubFollowingEndpoint` |
| **Fediverse author timeline** | Observable remote surfaces for a Contraponto author on Mastodon (or compatible): **remote profile / statuses** (outbox crawl) and the follower's **home** timeline after Follow/Accept (inbox backfill + live Creates). Both must show the same archive membership, ids, dates, and order. | feature v1.4 FQ13, FQ16 |
| **Protocol ingress** | Pre-matching rewrite of public federation URLs to internal `/__activity_pub__/...` JAX-RS tree via `ActivityPubIngressFilter` — prevents Friendica/Mastodon probes (`/api/nodeinfo`, `/{username}/poco`) from hitting `BlogEndpoint`. | `ActivityPubIngressPaths`, `ActivityPubIngressFilter` |
| **NodeInfo** | Optional instance discovery: `/.well-known/nodeinfo` JRD + `/nodeinfo/2.0` JSON (`protocols: activitypub`). | `ActivityPubNodeInfoService` |
...
| **Fediverse Create content** | Plain-text (or sanitized) body of a federated **Create** object's status text. **Main blog:** **title + canonical post link** (FQ3). **Secondary blog:** **title + blog name + canonical post link** (FQ20) — blog name distinguishes secondary posts under one Person. | Create / Note / Article content |
...
| Author appearance — Fediverse opt-in | Publish my blog posts to the Fediverse (all blogs — main and secondary) | Fediverse section — one control for all blogs (FQ25) |
