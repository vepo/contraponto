...
| **Blog subdomain** | Author-scoped host `{username}.{base-domain}` where the main blog home is `/` and posts are `/post/{slug}`; secondary blogs use `/{blogSlug}/…`. Author workspace hubs (`/writing`, `/manage`, `/administration`, …) and shell partials (`/components/menu`, …) are served on the same host without redirecting to the platform. URL mapping rules: [blog-subdomain-urls.md](blog-subdomain-urls.md). | `BlogSubdomainFilter`, `BlogPublicUrlService`, `APP_BLOG_SUBDOMAIN_BASE_DOMAIN` |
