...
| **Fediverse follow backfill** | When a **Fediverse follow** is **auto-accepted**, Contraponto enqueues **Create** deliveries for every published **main blog** post (oldest first) to that follower's remote inbox. | `ActivityPubDeliveryService.enqueueHistoricalPostsForAcceptedFollow` |
...
| **Fediverse post object URL** | Stable `id` and `url` on delivered **Note** / **Article** objects: platform host with **username prefix** (`/{username}/post/{slug}`), never author-subdomain paths — so remote clients (Feedverse, Friendica, …) resolve clickable links correctly. | `ActivityPubPaths.postObjectId` |
